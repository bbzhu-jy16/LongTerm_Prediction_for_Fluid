...
from util.arguments import Density2D_parse
from util.metrics import psnr_den_2D, cos_den_2D
from model.prediction_model import DensityPrediction2D
...
def train(args,model):
    ...
    density_data = np.asarray(density_data) #(64,64)
    density_data_input = np.zeros((density_data.shape[0],frame_num,res,res,1)) 
    for i in range(frame_num):
        density_data_input[:,i,:,:,0]=density_data
    
    model.compile(optimizer='adam',loss=density_loss)

    checkpointer = ModelCheckpoint(os.path.join(args.model_path,'2D_Long_density_prediction64_{epoch:03d}.h5'),verbose=1,save_weights_only=False,save_best_only=False)
...
def test(args, model):
    ...
    for filename in tqdm(filenames):
        ...
        #velocity_test = velocity_test['data'][frame0:frame0+frame_num,:,:,0,0:2]
        velocity_test = velocity_test['data'][frame0:frame0+frame_num,:,:,0:2]
        velocity_test = velocity_test.reshape(1,frame_num,res,res,2)
        #density_test0 = density_test[frame0,:,:,0,0]
        density_test0 = density_test[frame0,:,:]
        density_test0 = density_test0.reshape(res,res,1)
        #density_test_truth = density_test[frame0+1:frame0+frame_num+1,:,:,0,0]
        density_test_truth = density_test[frame0+1:frame0+frame_num+1,:,:]
        density_test_input = np.zeros((1,frame_num,res,res,1)) 
        for i in range(frame_num):
        ...
        for i in range(frame_num):
            for j in range(res):
                for k in range(res):
                    ...
                    err += (outputs_test[i,j,k]-density_test_truth[i,j,k])*(outputs_test[i,j,k]-density_test_truth[i,j,k])
        err /= frame_num*res*res
        err_all.append(err)
        psnr_frame = psnr_den_2D(density_test_truth, outputs_test)
        #psnr_scale_frame = psnr_scale(density_test_truth, outputs_test)
        psnr_all.append(psnr_frame)
        cos_all.append(cos_den_2D(density_test_truth, outputs_test))
        #psnr_scale_all.append(psnr_scale_frame)
...
def main():
    ...
    args=Density2D_parse()
    model=DensityPrediction2D(args.resolution,args.frame_num)
    mode=args.mode
    if mode=="train":
        train(args,model)
    elif mode=="test":
